 ...
 * XMLConfigBuilder是BaseBuilder的众多子类之一，它扮演的是具体的建造者的角色。XMLConfigBuilder主要负责解析mybatis-config.xml配置文件
 */
public class XMLConfigBuilder extends BaseBuilder {
  ...
  /**
   * 标识<environment>配置的名称，默认读取<environments>标签的default属性
   */
  ...
  public Configuration parse() {
    //判断是否已经完成了对mybatis-confi.xml配置文件的解析
    if (parsed) {
  ...
  /**
   * 解析<settings>节点，这个节点下的配置是mybatis的全局配置，它们会改变mybatis的运行时行为，在mybatis初始化时，
   * 这些全局配置信息都会被记录在configuration对象的对应属性中
   * @param context
   */
  ...
  /**
   * MyBatis每次创建结果对象的新实例时，它都会使用一个对象工厂（ObjectFactory）实例来完成，默认的对象工厂需要做的仅仅是实例化目标类，
   * 要么通过默认构造方法，要不在参数映射存在的时候通过参数构造方法来实例化，如果想覆盖对象工厂的默认行为，则可以
   * 创建自己的对象工厂来实现，改方法就是解析自创的对象工厂，并将其赋值到configuration对象中
   * @param context
   * @throws Exception
   */
  ...
  private void propertiesElement(XNode context) throws Exception {
    if (context != null) {
      ...
      //解析<properties>的resource和url属性，这俩个属性用于确定Properties配置文件的位置
